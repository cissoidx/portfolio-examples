...
from custom_loss import sigmoid_ce
...
def drop_path(x, drop_prob: float = 0., training: bool = False):
    ...
    # poptorch not support floor_, so replace it with bernoulli
...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.proj_dropout = Dropout(config.attention_probs_dropout_prob)
...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        for i in range(config.num_hidden_layers):
            recompute_checkpoint = i in [1,4,7,10]
