...
poprun --host [hostname1,hostname2,hostname3,hostname4] \
      --vv \
      --numa-aware=yes \
      ...
      --vipu-partition=pod64_vit \
      --vipu-cluster=[cluster_name] \
      --ipus-per-replica 4 \
      --executable-cache-path="/cachedir" \
      --num-replicas=16 --num-instances=4 \
      --vipu-server-timeout=600 \
      python train_from_scratch.py --config b16_in1k_pretrain --prefetch-depth 1 --gradient-accumulation 293 --batch-size 14 --dataloader-workers 96 --rebatch-size 2048
